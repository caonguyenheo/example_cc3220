...
static int HttpCore_HandleMethodLine(UINT16 uConnection, struct HttpBlob line, struct HttpBlob* pResource);
...
//        osi_Sleep(1000);
    ...
    if(g_state.listenSocket >= 0)
    {
      if (g_state.uOpenConnections < HTTP_CORE_MAX_CONNECTIONS)
      {
        ...
        else if((sock != SL_ERROR_BSD_EAGAIN) && (sock != SL_POOL_IS_EMPTY) && (sock != SL_ERROR_BSD_INEXE) && (sock != SL_ERROR_BSD_ENSOCK))
        {
        ...
        for (uConnection = 0; uConnection < HTTP_CORE_MAX_CONNECTIONS; ++uConnection)
        {
                    ...
                    g_state.connections[uConnection].connectionState == WebSocketDataRecv)
          {
            ...
            if(iRecvLen > 0)
            {
              ...
              int retVal = -1;
              retVal = HttpCore_HandleRequestPacket(uConnection, blob);
              if (retVal == 0)
              {
                // HttpDebug("handler packet failure\n");
...
//                HttpCore_CloseConnection(uConnection);
            ...
            else if((iRecvLen != SL_ERROR_BSD_EAGAIN) && (iRecvLen != SL_POOL_IS_EMPTY))
            {
              HttpDebug("\n Http Client close %d\n\r", iRecvLen);
              HttpCore_CloseConnection(uConnection);
              continue;
...
static int HttpCore_HandleRequestPacket(UINT16 uConnection, struct HttpBlob packet)
{
  ...
  //  int result;
  struct HttpBlob http_url = {0, NULL};
  int file_response = 0;
  ...
  while (1)
  {
    ...
    case DropCurrentHeader:
      ...
      break;
    case RequestMethod:
      HttpDebug("http req method \n");
      // If there is an error, then return error to drop the connection
      if (!HttpCore_HandleMethodLine(uConnection, line, &http_url))
        return 0;
      break;
    case RequestHeaders:
      HttpDebug("http req header \n");
      if (!HttpCore_HandleHeaderLine(uConnection, line)){
        HttpDebug("header line error\n");
        return 0;
      }
      break;
    ...
    case Processing:
      ...
      respCode = HandleCommand(http_url.pData, http_url.uLength, respBuffer, &file_response);
//            HttpDebug("http response code is %d \n", respCode);
//            HttpDebug("http response data: %s\n", respBuffer);
//            HttpDebug("http response file: %d\n", file_response);
      SendResponse(uConnection, respCode, respBuffer, file_response);
      //      HttpDebug("Finish processing message at LINE(%d)\n", __LINE__);
      break;
    case ResponseData:
    ...
    case ResponseComplete:
      ...
      if(file_response == -1)
      {
        HttpDebug("switch to STATION mode\n");
        return -1;
      }
      HttpDebug("complete session\r\n");
...
static int HttpCore_HandleMethodLine(UINT16 uConnection, struct HttpBlob line, struct HttpBlob* pResource)
{
  ...
  // HttpDebug("method Header: %.*s \n\r", versionLocation - methodLocation - uMethodLength - 2, methodLocation + uMethodLength + 1);
  
  
  // Find the URL part of the header
  pResource->pData = methodLocation + uMethodLength + 1;
  pResource->uLength = (UINT16)(versionLocation - (methodLocation +  uMethodLength + 1) - 1);
  HttpDebug("method Header: %.*s \n\r", pResource->uLength, pResource->pData);
...
static void HttpCore_ProcessNotFound(UINT16 uConnection)
{
  ...
  HttpResponse_CannedError(uConnection, HTTP_STATUS_OK);
  // HttpResponse_CannedError(uConnection, HTTP_STATUS_OK);
}
